...
class LateMiniMaxPlayer(Player):

    MIN_VALUE = 15
    MAX_VALUE = 40

    def __init__(self, color:DiscEnum, threshold:float = 0):
        if threshold == 0:
            self.threshold = random.choice(range(LateMiniMaxPlayer.MIN_VALUE, LateMiniMaxPlayer.MAX_VALUE))/100
        super().__init__(color)

    def play(self, board: Board) -> Point:
        if board.get_empty_spots() < int(self.threshold * 64):
            ...
            best_move, _ = self.minimax(board, False, opponent_color)
